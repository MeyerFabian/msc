...
\begin{enumerate}
  ...
  \item Solve the governing partial differential equations on the grid using numerical methods.
  \item Transfer back grid data to particles.
\end{enumerate}
Based on PIC to reduce dampening and momentum conservation Fluid-Implicit-Particle (FLIP) was developed for fluid simulations and is still widely in use for large simulations. Their counterpart being the empirical Smoothed Particle Hydrodynamics (SPH) \cite{gingold1977smoothed}.

For that matter APIC was developed as an improvement to PIC and FLIP solvers. Due to the definition of a local affine velocity field around each particle (instead of a single vector), APIC conserves angular momentum, reduces damping (of the underlying numerics) and improves stability over PIC and FLIP, more in chapter \ref{sec:apic}. \cite{MPM:APIC} \cite{MPM:OLD_APIC}
...
\textbf{Material Point Method:} The MPM-Method is like the PIC-Method very general in that it uses analysis and numerics to be derived without the need for empirical assumptions. Originally it was developed by \cite{sulsky1995application} with a focus on the dynamics of solids. \cite{MPM:SNOW} brought the method into computer graphics for their animation film \textit{Frozen}. In rapid succession a variety materials and effects got modeled by the MPM-Method. Since MPM is based on discretizing governing equations and using constitutive models covered by the different literature its fast rate of expenditure is not a surprise. To give a short summary of the developments in computer graphics:
...
\textbf{Particle sorting:} \cite{PIC:GPU} studies different particle list sorting methods. After a particle position update becomes partially sorted. There exist methods like 'Message Passing Sort' and 'In Place Particle-QuickSort' to try to utilize the partial sorting and save time. \cite{PIC:GPU} also uses fixed bin-sizes which can be, depending on the extend of use, quite memory intensive. Fixed size binning certainly make the methods mentioned before easier to implement, which don't necessarily map well to the GPU due to its high dependence on coalescing. As a result they come to the conclusion to fall back to the CUDA library \texttt{thrust}. It provides the radix sort method \texttt{thrust::sort\_by\_key()}. Full or index sort can be done. A full sort reorders every variable of a particle. An index sort gives back an index with the offset in the particle list.
\cite{NVIDIA:NNSEARCH} compare radix to counting sort utilizing a uniform grid and uses the latter for SPH-fluids. This master thesis implements counting sort.

\textbf{Particle-Grid transfers:} The particle to grid transfer are generally done in two manners \cite{stantchev2008fast}:
\begin{enumerate}
...
\textbf{Particle Activation:} If a particle is under a user-defined velocity threshold a particle becomes deactivated. It does not get recognized by any particle to grid transfer (nor any grid transfer not affected by the velocity transfer). They may of course be reactivated if the back transfer of the velocity exceeds that threshold again or they collide. Dividing the particles up can be realized with stream compaction algorithms: One implementation would be to compute a scan (based on the discussed condition) followed up by a modified reorder shader discussed in chapter \ref{sec:implementation}. The modified reorder shader would then be coalesced. \cite{MPM:GPU}
...
\begin{enumerate}\label{ref:itemize_conv}
  \item\label{item:refl_free} $\boldsymbol{U}$,$\boldsymbol{V}$ are reflection-free corresponding to true rotation matrices, i.e. both $\text{det}(U), \text{det}(V) = 1$ hold.
...
\end{equation}
The linear system of eq. \ref{eq:linear_system} can be solved with the conjugate gradient method (Algorithm \ref{alg:conj_grad}) since $\frac{\partial g}{\partial v}$ is symmetric, positive definite. This is due it being the Hessian of a convex function $E$. There is to important things notice:
\begin{enumerate}
