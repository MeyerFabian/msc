...
\textbf{Parallel reduction}, \textbf{map} and \textbf{scan} are tested against \texttt{std::transform\_reduce} as it supplies the same functionality as a fully reduced parallel reduction. It is standard since C++17.
\begin{table}[h]
...
\begin{table}
  \begin{tabular}{ | l | l | l | l | l | l | l |}    \hline
    Layout &  $\Delta t_c (\mu s)$ & Speedup &VRAM     & SM     & L2     & Tex \\\hline
    AoS         & 278                   & -       &77.7\%   & 7.3\%  & 30.3\% & 14.7\% \\\hline
    SoA         & 122                   & 2.26x   &75.4\%   & 14.3\% & 29.4\% & 17.9\% \\
    \hline
  \end{tabular}
\caption{Timings betweens different buffer layouts. Map operation with one instruction \texttt{float x = length(vel)} on $1024 \times 1024$ \texttt{vec4} buffer storing position and velocity (\texttt{vel})}.}\label{table:map_1}
\end{table}
...
\begin{table}
  \begin{tabular}{ | l | l | l | l | l | l | l |}    \hline
    ...
    \hline
  \end{tabular}
\caption{Timings betweens different buffer layouts. Map operation with instructions \texttt{float x = length(vel), float y = length(pos)} on $1024 \times 1024$ \texttt{vec4} buffer storing position(\texttt{pos}) and velocity (\texttt{vel})}.}\label{table:map_1}
\end{table}
...
\begin{table}
  \begin{tabular}{ | l | r | r | r | r | r |r|}    \hline
    Best Case(*)          &  $\Delta t_c$         &VRAM  & Top Stall Reas. & Read & Write\\\hline
    Index                 &  1,516                &25.6\%& L. Scoreb.(78.7\%) &&\\\hline
    Full(8 vec4)     &  1,731                &75.3\%& L. Scoreb.(92.2\%) &&\\
    \hline
    \hline
    Worst Case(*)          &  $\Delta t_c$         &VRAM              & Top Stall Reas.    &Read & Write\\\hline
    Index                  &  1,516                &26.0\%            & L. Scoreb.(73.5\%) & &\\\hline
    Deep Sort(8 vec4)      &  9,847                &24.0\%            & L. Scoreb.(82.2\%) & &\\
    \hline
 \end{tabular}
\caption{Order dependency of binning of $1024\times1024$ randomly positioned particles in a $128\times128\times128$ grid.}\label{table:bin}
\end{table}
