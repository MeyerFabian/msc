...
\begin{algorithm}[t]
  \caption{G2P-Transfer}\label{alg:conj_grad}
  \begin{algorithmic}[1]
    \FORALLP{$blockNode \in grid$}
      \State \text{shared vec4 } $blockAndHalo$ [$H\_FLAT$]
      \ForAll{$node\_id \in \text{assignedNodes}(blockNode, H\_FLAT)$}
      \State $blockAndHalo[node\_id] \gets \text{LoadGlobalValue}(node\_id)$
      \EndFor
      \State \text{barrier()}
      \ForAll{$particle \in \text{bin}(blockNode)$}
      \State $sum \gets$ vec4(.0)
      \ForAll{$neighbor \in \text{support}(blockNode)$}
      \State $w_{ip} \gets weight(particle,neighbor)$
      \State $sum \gets sum + blockAndHalo[neighbor]*w_{ip}$
	\EndFor
      \State $particle \gets sum$
      \EndFor
    \ENDFAP
  \end{algorithmic}
\end{algorithm}
...
\begin{figure}[htbp]
  \incl[svgpath =images/,width=\textwidth]{indirect_dispatch}
  \caption{dwad}
