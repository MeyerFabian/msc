...
As an extension to this process batching is introduced. Instead of conservatively loading one particle at a time, each thread handles multiple particles together which it stores in register memory. Important to this process is to unroll the corresponding code that loads as well as processes these multiple particles. Instead of needing to rerun the transfer for each particle, the number of runs now done is the number of non-batched runs divided by the number of particles. This process adds to the amount of registers allocated and can thus also negatively impact performance.
\subsubsection{Particle-to-Grid-transfers}\label{sec:p2g}
The Particle-to-Grid transfer is
\begin{figure}[htbp]
    \centering
...
\begin{algorithm}[htpb]
  \caption{P2G-Transfer (synchronized)}\label{alg:p2g}
  \begin{algorithmic}[1]
    \FORALLP{$blockNode \in grid$}
    \State \text{shared vec4 } $blockAndHalo$ [$H\_FLAT$]
    \ForAll{$local_id\_id \in \text{assignedNodesID}(blockNode, H\_FLAT)$}
      \State $blockAndHalo[node\_id] \gets $vec4(.0)
      \EndFor
      \State \text{barrier()}
      \ForAll{$particle \in \text{bin}(blockNode)$} \Comment{global load}
      \ForAll{$neighbor\_id \in \text{support}(blockNode)$} \Comment{transfer}
	\State $w_{ip} \gets weight(particle,neighbor\_id)$
	\State $blockAndHalo[neighbor\_id] $ += $ particle*w_{ip}$
	\State \text{barrier()} \Comment{uniform flow control needed!}
	\EndFor
      \EndFor
      \ForAll{$(node,local\_id) \in \text{assignedNodes}(blockNode, H\_FLAT)$}
      \State atomicAdd($node$,$blockAndHalo[local\_id]$) \Comment{global write}
      \EndFor
    \ENDFAP
  \end{algorithmic}
\end{algorithm}
