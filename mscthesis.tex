...
\end{equation}
The energy produced by equation \ref{eq:linear_elas} using the small strain tensor is not rotationally invariant, w.r.t. to $\mathbf{F}$: $\psi(\mathbf{R}_{0} \mathbf{F}) \neq \psi(\mathbf{F})$. The deformation caused by stress however does not affect rigid body motions. Given the polar decomposition $\mathbf{F} = \mathbf{RS}$, an alternate strain measure may be defined as:
\begin{equation}
  \hat { \boldsymbol { \epsilon } } ( \mathbf { F } ) = \boldsymbol { \epsilon } \left( \mathbf { R } ^ { T } \mathbf { F } \right) = \frac { 1 } { 2 } \left( \mathbf { R } ^ { T } \mathbf { F } + \left( \mathbf { R } ^ { T } \mathbf { F } \right) ^ { T } \right) - \mathbf { I } = \mathbf { S } - \mathbf { I }
\end{equation}
Substituting $\hat{\boldsymbol{\epsilon}}$ into equation \ref{eq:linear_elas} leads to the energy definition of corotational hyperelasticity using rotation-invariance of Frobenius norm and trace:
\begin{equation}
  \Psi = \mu \| \mathbf { F } - \mathbf { R } \| _ { F } ^ { 2 } + \frac { \lambda } { 2 } \operatorname { tr } ^ { 2 } \left( \mathbf { R } ^ { T } \mathbf { F } - \mathbf { I } \right)
\end{equation}

\begin{equation}
  \Psi(\boldsymbol{\Sigma}) = \mu \| \boldsymbol { \Sigma } - \mathbf { I } \| _ { F } ^ { 2 } + \frac { \lambda } { 2 } \operatorname { tr } ^ { 2 } ( \boldsymbol { \Sigma } - \mathbf { I } )
\end{equation}
