...
\end{equation}
I.e. this component is only evaluated if $i = j$. In this case it becomes equivalent to the identity matrix $\boldsymbol{I}$.
...
\begin{equation}\label{eq:j}
  d_tV = {^t_0J} d_0V.
\end{equation}
The area change is given by Nanson's Formula. Where $\boldsymbol{A}$ points in the direction of the normal of the area:
...
\end{equation}
A surface intergral may then be transformed to current configuration by
\begin{equation}
...
\end{equation}
where $_0h = h(_0\boldsymbol{x},\cdot)$ is the pull back of $_th = h(_t\boldsymbol{x},\cdot)$.
...
\end{equation}
The energy produced by equation \ref{eq:linear_elas} using the small strain tensor is not rotationally invariant, w.r.t. to $\boldsymbol{F}_E$: $\psi(\boldsymbol{\epsilon}(\boldsymbol{R}_{0} \boldsymbol{F}_E)) \neq \psi(\boldsymbol{\epsilon}(\boldsymbol{F}_E))$. Rigid body motions however don't result in deformation and consequently don't need to be recovered from. So energy should not change. Given the polar decomposition $\boldsymbol{F}_E = \boldsymbol{R}_E\boldsymbol{S}_E$, an alternate strain measure may be defined as:
\begin{equation}
