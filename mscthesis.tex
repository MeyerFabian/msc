...
$$
\begin{equation}
  =  \int _ { \Omega ^ { t } } \frac{\partial _tq _ { i }}{\partial _tx_k} ( _t\boldsymbol { x } , t )\sigma _ {ik} d_t\boldsymbol{x}.
\end{equation}
...
Its is very important that the two transfers as well as the grid solver are in compliance with the governing equations. While the grid solver will be derived from the weak form of the governing equation the transfers also need to be chosen in a way that also conserve the properties defined in them.
\cite{MPM:COURSE}
...
\begin{appendices}
  \section{Inertia Tensor for Cubic Splines}
  \begin{code}
...
\end{code}
  ...
  \left[
    \begin{array}{ccc}
      ...
      0.         &0.33333333 &0.	\\
      0.         &0.         &0.33333333\\
  \end{array}\right]=\boldsymbol{D}_p
  $$
\end{appendices}
\clearpage
\listoffigures
