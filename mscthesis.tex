...
\begin{equation}\label{eq:piola}
  \boldsymbol{P} = \boldsymbol{\sigma}\boldsymbol{F}^{-T}J
\end{equation}
...
\end{equation}
\cite{MPM:APIC}\cite{MPM:COURSE}
\subsubsection{APIC momentum Transfers}
...
\end{equation}
  ...
  \item Grid to Particle Transfer (in a $PIC$-manner):
$$
    \boldsymbol { B } _ { p } ^ { n + 1 } = \frac { 1 } { 2 } \sum _ { i } w _ { i p } ^ { n } (  { \boldsymbol { v } } _ { i } ^ { n + 1 } ( \boldsymbol { x } _ { i } ^ { n } - \boldsymbol { x } _ { p } ^ { n } +  { \boldsymbol { x } } _ { i } ^ { n + 1 } - \boldsymbol { x } _ { p } ^ { n + 1 } ) ^ { T }
    $$
    \begin{equation}\label{eq:apic_mom_g2p}
+ ( \boldsymbol { x } _ { i } ^ { n } - \boldsymbol { x } _ { p } ^ { n } -  { \boldsymbol { x } } _ { i } ^ { n + 1 } + \boldsymbol { x } _ { p } ^ { n + 1 } ) (  { \boldsymbol { v } } _ { i } ^ { n + 1 } ) ^ { T } )
    \end{equation}
    \begin{equation}
...
\end{equation}
A simple proof (Appendices: \ref{app:dp_proof}) cancelling out the numerous polynomials can be done for instance using \textbf{SymPy} (\cite{Sympy}).
\cite{MPM:APIC}\cite{MPM:OLD_APIC}
...
Any conservative integrator could be used to discretize time. The implicit midpoint rule ($\lambda =\frac{1}{2}$) is one of such
...
\end{equation}
Replacing the Lagrangian acceleration $_0a_\alpha$ with the left side of \ref{eq:midpoint} using velocity, taking care of the right side and pushing forward to Eulerian view:
...
Starting from the last update on the grid which is typically the position with eq. \ref{eq:midpoint}:
$$
\frac{\partial _t\boldsymbol{x}}{\partial t} = {_t\boldsymbol{v}} \Rightarrow \frac{\boldsymbol{\hat{x}}_i^{n+1} - \boldsymbol{x}_i^{n}}{\Delta t} = \boldsymbol{v}_i^{n+\lambda} =  (1-\lambda)\boldsymbol{v}_i^n + \lambda\boldsymbol{\hat{v}}_i^{n+1}
  $$
\begin{equation}
  \Leftrightarrow \boldsymbol{\hat{x}}_i^{n+1}   =\boldsymbol{x}_i^{n} + \Delta t \left((1-\lambda)\boldsymbol{v}_i^n + \lambda\boldsymbol{\hat{v}}_i^{n+1}\right).
\end{equation}
Due to \ref{eq:space_discr} a velocity update can be put together as
\begin{equation}
  \frac{1}{\Delta t} (({m^n\boldsymbol{\hat{v}}^{n+1}})_i-({m^n\boldsymbol{v}^n})_i)
  = \boldsymbol{f}_i^{n+\lambda}
 \Leftrightarrow
 \boldsymbol{\hat{v}}^{n+1}_i
 = \boldsymbol{v}^n_i+ \frac{\Delta t}{m_i^n}\boldsymbol{f}_i^{n+\lambda},
\end{equation}
where the force can be also described in terms of the in-between point:
\begin{equation}
 \boldsymbol{f}_i^{n+\lambda} = \boldsymbol{\sigma}^{n+\lambda}_p(\boldsymbol{F}_p^{n+\lambda}(\boldsymbol{\hat{x}}_i^{n+1})).
\end{equation}
