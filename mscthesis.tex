...
\begin{enumerate}
  \item \textbf{Particle Push:} X particles in a local neighborhood of a grid point push their attributes onto it. Since this relationship results in a race condition \texttt{atomicAdd(float f)}s or explicit synchronization is necessary.
...
\begin{itemize}
  ...
  \begin{itemize}
  \item \texttt{unary\_op(x) := length2(x)} and
...
\clearpage
\subsection{Counting Sort \& Stream Compaction}
As the sorting algorithm of choice counting sort is used as in \cite{NVIDIA:NNSEARCH}. Counting sort is split into three parts:
\begin{enumerate}
  \item
\parbox[t]{\dimexpr\textwidth-\leftmargin}{%
 ...
 \includesvg[svgpath =images/,width=0.425\textwidth]{binning}
  \caption{Binning: Particle adds one to the grid counter after storing it.}
  \label{fig:binning}
\end{wrapfigure}
  ...
  \item \textbf{Reordering}: $i_p := s_i + o_p$ will now lead to a new ordering of the particle indices. Sorting is now similar to methods \texttt{sort\_by\_key(uint i)}, where ordering of particles within the same bin is undefined. Supported is either a full deep copy of all particle variables or alternatively an indexing to access an existing particle buffer in the specified sorted manner.
\end{enumerate}
The one-dimensional indexing of the grid nodes can be chosen as preferred. Possible are a per-block indexing or a global indexing which are both supported. \cite{NVIDIA:NNSEARCH}
\subsection{MPM-Transfers}\label{sec:transfers}
...
\begin{wrapfigure}{r}{0.55\textwidth}
  \includesvg[svgpath =images/,width=0.55\textwidth]{block_transfer_inv_par}
  \caption{dwad}
...
\item Subtract the global scan's first element of every row-element and limit your view to indices $b,c$ then:
  $$
  T_{st} = S^G_{st} - \left(\sum_{q,r=0}^{s-1,n} a_{qr} + \sum_{r=0}^{(j-1)} a_{sr}\right)
  $$
\begin{equation}
  \Rightarrow T_{bc} = \sum_{r=j}^{(c-1)} a_{br}
\end{equation}
\item Add the last value of $\boldsymbol{L}$ of the previous row (if it exists) to the current row.
  \begin{equation}
    \Rightarrow T^2_{bc} = a_{(b-1)(n-l)} + \sum_{r=j}^{(c-1)} a_{br}
  \end{equation}
