...
\end{table}
The second optimization is giving these algorithms more sequential work. This will lighten the SM throughput and increase VRAM throughput as can be seen in both cases. Note, that each thread in table \ref{tab:map_reduce} and \ref{tab:scan} does the same amount of parallel work; the sequential work is additionally being done. However due to the additional sequential work parallelization a block level goes down by the same factor. As mentioned in \cite{NVIDIA:PARALLEL_REDUCTION} this balances the relation of I/O latency to work better and thus improves performance.
