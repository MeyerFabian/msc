...
\end{enumerate}
The grid to particle transfer in generally are easier since a particle already knows, due to its position, the grid points in its local neighborhood. The grid points in a uniform grid are 'sorted' by nature.

Generally these methods are augmented by splitting the grid in 2D/3D tiles making use of the shared memory structure of the GPU \cite{PIC:GPU} \cite{honig2010generic} \cite{MPM:GPU}. Shared memory on GPUs typically ranges from 48-64KB.  The grid nodes reached by the support of the interpolation function on each side also have to be loaded into memory, typically called the halo of a tile. Since the number of particles per grid point in the MPM-method is generally between 4-10 only very limited tile-sizes are available \cite{MPM:SNOW}.
An interesting prospect for filtering techniques is given by NVIDIA's new shuffle operations, which allows to 'share' register memory between threads of a warp \cite{NVIDIA:SHUFFLE}. They are however not supported on other architecture yet.
